...
The system SHALL keep learner-facing serialization and governed delivery free from staff-only lineage warnings, administrative historical-safety metadata, and staff governance issue payloads unless a separate learner-safe requirement explicitly adds them later.
...
#### Scenario: Learner requests a staff lineage and safety surface
- **WHEN** a learner or student user attempts to access staff lineage or safety visibility
- **THEN** the system SHALL deny access or withhold the staff-only data according to existing authorization boundaries
- **AND** learner-facing surfaces SHALL not render historical warning controls
