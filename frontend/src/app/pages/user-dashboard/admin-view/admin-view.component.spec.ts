...
function buildGovernanceSummary(overrides: Partial<any> = {}) {
  return {
    ...
    course_title: overrides['course_title'] || 'Biology',
    publish_readiness: {
      course_id: 'course-1',
      course_title: 'Biology',
      is_ready: true,
...
describe('AdminViewComponent', () => {
  ...
  it('uses EchoLoadingState while admin data is loading', () => {
    ...
    coursesService.governanceSummaryResponse = new Subject<any>().asObservable();
  ...
  it('keeps existing admin links and actions present', () => {
    ...
    expect(compiled.textContent).toContain('Managed in admin tools');
    expect(compiled.textContent).not.toContain('Delete');
    expect(compiled.textContent).not.toContain('Publish Course');
  });

  it('uses flagship-aligned admin announcement copy', () => {
    usersService.usersResponse = of([{ id: 'user-1', firstname: 'Grace', lastname: 'Hopper', role: 'admin', created_at: '2026-05-01T00:00:00.000Z' }]);
    coursesService.coursesResponse = of([{ id: 'course-1', title: 'Biology' }]);
    usageStatsService.usageResponse = of([{ label: 'Daily Active Users', value: 82, color: 'bg-primary' }]);
    analyticsService.overviewResponse = of({ totals: { students: 14, teachers: 2, courses: 5, pending_enrollments: 1 } });
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    expect(compiled.textContent).toContain('Introduction to Africa is ready for the guided learner, teacher, and admin walkthrough.');
    expect(compiled.textContent).toContain('Use the governance summary and runtime recommendations to narrate publish safety, evidence integrity, and learner support.');
    expect(compiled.textContent).not.toContain("Don't forget the summit tomorrow!");
    expect(compiled.textContent).not.toContain('New Course: Web Development');
  });
  ...
  it('renders blocked publish readiness issues and retries read-only loading', () => {
    usersService.usersResponse = of([{ id: 'user-1', firstname: 'Grace', lastname: 'Hopper', role: 'admin', created_at: '2026-05-01T00:00:00.000Z' }]);
    coursesService.coursesResponse = of([{ id: 'course-1', title: 'Biology' }]);
    coursesService.governanceSummaryResponse = throwError(() => new Error('publish readiness failed'));
    usageStatsService.usageResponse = of([{ label: 'Daily Active Users', value: 82, color: 'bg-primary' }]);
    analyticsService.overviewResponse = of({ totals: { students: 14, teachers: 2, courses: 5, pending_enrollments: 1 } });
